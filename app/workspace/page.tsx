  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
      <div className={`${isLeftSidebarCollapsed ? 'sticky  z-50' : ''} flex bg-[#f9f9f9] top-0 h-full w-full border-b border-[#ededed]`}>
        <DashboardHeader
          title="Classes"
          description="Select a class to manage its exams, subjects, and questions."
        >
          <button
        ...
        {isLoading ? (
          ...
          filteredClasses.map(cls => (
            ...
            >
              <div className="px-4 py-3 flex flex-col md:flex-row items-center justify-between gap-4">
                {/* Class info */}
                <div
                  className="flex items-center gap-3 flex-1 w-full cursor-pointer"
                  onClick={() => router.push(`/workspace/classes/${cls.id}/exams`)}
                >
                  <span className="flex items-center justify-center bg-zinc-300/20 text-[#0e0f10] rounded-sm px-1.5 py-1 group-hover:bg-blue-50 group-hover:text-blue-600 transition-colors">
                    <GraduationCap size={13} />
                  </span>
                  <h3 className="text-sm font-medium text-[#0e0f10] tracking-tight group-hover:text-blue-600 transition-colors">
                    {cls.name}
                  </h3>
                </div>
          ...
          <div className="text-center py-20 bg-white rounded-sm border border-dashed border-zinc-400/30 flex flex-col items-center">
            ...
            <p className="text-xs text-[#6b6b6b] max-w-xs mx-auto mb-8 leading-relaxed">
              Start by creating your first class to organize your assessment structures.
            </p>
            <button
              onClick={() => handleOpenModal()}
